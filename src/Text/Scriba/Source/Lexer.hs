...
-- TODO: do I instead want to recognize & only at the beginning of a
-- line? Of course I'd _like_ to be able to recognize an & that is
-- genuinely the start of a new block beforehand. Then I'd be able to
-- insert braces into the token stream, metaphorically speaking. The
-- issue is that verbatim text messes that up.
--
-- I suppose we should instead simply have our whitespace
-- (i.e. form-separator) parsers be indentation-aware? That's very
-- similar to our current parsers, I suppose.
pAmp :: Parser Tok
pAmp = "&" $> Amp
